...
<li class="form"><form action="blog.php" method="get">
...
if ($fu->db->NumRows() > 0) {
    while($row = $fu->db->ReadRecord()) {
		...
		if (isset($_GET['cat']) && $_GET['cat'] == $row['cat_id']) { echo ' selected="selected"'; }
...
<input type="text" id="title" name="title" size="35" maxlength="50" value="<?php echo $post->getTitle(); ?>" /></p>

<p><textarea id="body" class="wysiwyg" name="body" cols="80" rows="18"><?php echo htmlspecialchars($post->getBody()); ?></textarea></p>
...
if ($fu->db->NumRows() > 0) {
    ...
    while($row_cat = $fu->db->ReadRecord()) {
        ...
        if (array_key_exists($row_cat['cat_id'], $post->getCategoryArray())) { echo ' checked="checked"'; }
        echo " />\n";
...
<div class="secondary">
<input type="submit" name="action" value="delete" class="delete" title="Delete this entry, accesskey x" accesskey="x" />
<?php } else { ?>
<input type="submit" name="action" value="add" class="add" title="Add this entry, accesskey s" accesskey="s" />
<?php } ?>
</div>
</fieldset>
</div>
